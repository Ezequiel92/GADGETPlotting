...
# Arguments
- `data::Dict{String, Vector{Float64}}`: Return values of the [`get_cpu_txt`](@ref) function.
- `title::String = ""`: Title for the figure. If an empty string is given no title is 
  printed, which is the default.
...
"""
    cpu_txt_plot(
        data::Vector{Dict{String, Vector{Float64}}},
        labels::Array{String, 2}; 
        <keyword arguments>
    )::Plots.Plot
...
# Arguments
- `data::Vector{Dict{String, Vector{Float64}}}`: Vector of return values of the
...
function cpu_txt_plot(
    ...
    percentages = [first(values(dict)) for dict in data]
