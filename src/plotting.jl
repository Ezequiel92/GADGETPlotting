...
- `axes::Bool = false`: If true, the axes passing through (0.0, 0.0) are drawn. If false, 
  no axes are drawn.
- `axes_color::Symbol = :white`: Color of the axes passing through (0.0, 0.0), only relevant 
  if `axes = true`.
...
Make a plot of columns `y_axis` vs. column `x_axis`, of the sfr.txt text file.
...
# Arguments
- `data::Dict{Union{Int64, String}, Any}`: Return values of the sfr_txt_data function.
...
Make a plot comparing among several simulations `y_axis` vs. column `x_axis`, 
of the sfr.txt text file.
...
# Arguments
- `data::Vector{Dict{Union{Int64, String}, Any}}`: Vector of return values of the 
  sfr_txt_data function.
- `x_axis::Int64`: Column number for the x axis.
...
function sfr_txt_plot(
    ...
    if scale[2] == :log10
        ...
        if length_short_cases >= 2
            ...
            # Fill the datasets with NaN so all have the same length.
            max_length = maximum(length.(y))
            for (i, l_r) in enumerate(length.(y))
                if l_r < max_length
                    @inbounds for _ in 1:(max_length - l_r)
                        push!(x[i], NaN)
                        push!(y[i], NaN)
...
# - The plot generated by the PGFPlotsX backend of Plots.jl.
# """
# function kennicutt_schmidt_plot2(
#     gas_mass_data::Dict{String, Any},
#     star_mass_data::Dict{String, Any},
#     age_data::Dict{String, Any},
#     pos_data::Dict{String, Any},
...
#         replace(
#             string(density_unit), 
#             "M⊙" => L"$\Sigma_\mathrm{gas} \, / \, \mathrm{M_{\odot} \,",
#         ),
#         "^-2" => L"^{-2}}$",
#     )
...
#     if clock > 1e9
#         println()
#         println("Density data:")
#         display(density_data)
#         println()
#         println("Minimum: ", minimum(density_data))
#         println()
#         println("SFR data:")
#         display(sfr_data)
#         println()
#         println("Minimum: ", minimum(sfr_data))
#         println()
#     end
...
# 	KS_intercept = ustrip(Float64, sfr_unit, KENNICUTT98_INTERCEPT)
# 	KS_intercept *= ustrip(Float64, density_unit, KENNICUTT98_RHO_UNIT)^(-KENNICUTT98_SLOPE)
# 	y_KS_98 = @. log10(KS_intercept) + KENNICUTT98_SLOPE * log10(sorted_density)
...
#     # Annotations with the fitted parameters and its errors.
#     annotate!(
#         relative(figure, 0.05, 0.95, log = (true, false))...,
#         text(
#             L"\Sigma_\mathrm{SFR} = \mathrm{A}\,\Sigma_\mathrm{gas}^{\,\mathrm{N}}", 
#             20, 
#             :left,
...
#     )
#     annotate!(
#         relative(figure, 0.05, 0.9, log = (true, false))...,
#         text(
#             L"\mathrm{N} = %$slope \pm %$slope_error", 
#             "Courier", 
#             20, 
...
#     )
#     annotate!(
#         relative(figure, 0.05, 0.85, log = (true, false))...,
#         text(
#             L"\mathrm{log}_{10}(\mathrm{A}) = %$intercept \pm %$intercept_error",
#             20, 
#             :left,
