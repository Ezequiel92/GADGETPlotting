...
# Arguments
- `z::Union{Vector{Float64}, Nothing}`: Vector with the data to be mapped, if `nothing`
  the density will be mapped.
- `position_data::Dict{String,Any}`: Return value of the [`get_position`](@ref) function.
...
function compare_simulations_plot(
    ...
    elseif x_quantity == "gas_mass" || x_quantity == "dm_mass" || x_quantity == "star_mass"
        ...
        if x_factor != 0
            ...
            x_unit = replace(unit, "M⊙" => "\\mathrm{M}_{\\odot}")
        end
    ...
    elseif y_quantity == "gas_mass" || y_quantity == "dm_mass" || y_quantity == "star_mass"
        ...
        if y_factor != 0
            ...
            y_unit = replace(unit, "M⊙" => "\\mathrm{M}_{\\odot}")
        end
    ...
    if x_unit === nothing
        xlabel = L"\mathrm{%$xlabel}"
    else
        xlabel = L"\mathrm{%$xlabel} \, / \, \mathrm{%$x_unit}"
    end

    if y_unit === nothing
        ylabel = L"\mathrm{%$ylabel}"
    else
...
function density_histogram_plot(
    ...
    annotate!(
        relative(figure, 0.5, 0.95, log = (false, y_scale == :log10))...,
        text("$clock $time_unit", "Courier", 22, :center),
    )
...
# Arguments
- `x_data::Vector{Float64}`: Return value of the [`get_fmol`](@ref) or [`get_fatom`](@ref) function.
- `time::Unitful.Quantity`: Time with units for the time stamp of the plot. 
  All available time units in [Unitful](https://github.com/PainterQubits/Unitful.jl) and [UnitfulAstro](https://github.com/JuliaAstro/UnitfulAstro.jl) can be used, e.g. UnitfulAstro.Myr.
...
- The plot generated by the GR backend of [Plots.jl](https://github.com/JuliaPlots/Plots.jl).
"""
function fraction_histogram_plot(
    x_data::Vector{Float64},
    time::Unitful.Quantity,
    x_label::String;
    bins::Int64 = 20,
    ...
    figure = histogram(
        x_data,
        bins = range(0, stop = maximum(x_data), length = bins),
        xlabel = x_label,
        yscale = y_scale,
        normalize = :probability,
        size = (1000, 800),
...
Make a plot comparing `y_quantity` vs. `x_quantity`, taken from the function [`quantities_2D`](@ref).
...
- "OH" (12 + log10(oxygen_mass / hydrogen_mass))
...
# Arguments
- `data::Vector{Dict{String,Any}}`: Return value of the [`quantities_2D`](@ref) function.
- `x_quantity::String`: Physical magnitude for the x axis. 
- `y_quantity::String`: Physical magnitude for the y axis.
- `units::Dict{String, Unitful.FreeUnits{N, D, nothing} where {N, D}}`: Dictionary with the 
  units of the quantities in `data`. It has to have three keys: "mass", "length" and "time", 
  each pointing to the corresponding unit.
- `title::String = ""`: Title for the figure. If an empty string is given no title is 
  printed, which is the default.
- `x_factor::Int64 = 0`: Numerical exponent to scale the `x_quantity`, e.g. if `x_factor` = 10 
  the corresponding axis will be scaled by ``10^{10}``. The default is no scaling.
...
function quantities_2D_plot(
    ...
    # Data order check
    (
        x_quantity ∈ ["STARS", "P", "GAS", "SFR", "SSFR", "SFE", "OH", "Psi_FMOL", "COLD_GAS"] ||
        error("`x_quantity` must be one of the available quantities")
    )
    (
        y_quantity ∈ ["STARS", "P", "GAS", "SFR", "SSFR", "SFE", "OH", "Psi_FMOL", "COLD_GAS"] ||
        error("`y_quantity` must be one of the available quantities")
    )

    # Extract data and scale it by 10^factor
    x_data = copy(data[x_quantity])
    y_data = copy(data[y_quantity])

    # Data filtering
    deleteat!(x_data, y_data .=== missing)
    filter!(z -> z !== missing, y_data)
    deleteat!(y_data, x_data .=== missing)
    filter!(z -> z !== missing, x_data)

    if scale[2] == :log10
        deleteat!(x_data, y_data .<= 0.0)
        filter!(z -> z > 0.0, y_data)
    end
    if scale[1] == :log10
        deleteat!(y_data, x_data .<= 0.0)
        filter!(z -> z > 0.0, x_data)
    end

    # If there are 2 or less data point go back to linear scale
    if length(x_data) <= 2
        x_scale = :identity
        y_scale = :identity
        x_data = Float64[]
    ...
    # Unit formatting for the x axis
    if x_quantity ∈ ["STARS", "GAS", "COLD_GAS"]

        unit = string(units["mass"] / units["length"]^2)
    ...
    elseif x_quantity ∈ ["SSFR", "SFE", "Psi_FMOL"]

        unit = string(units["time"]^-1)

        if x_factor != 0
            x_unit = replace(
                "\\left(10^{$x_factor} \\, \\mathrm{$unit",
                "^-1" => "^{-1}}\\right)",
            )
        else
            x_unit = replace(
    ...
    elseif x_quantity ∈ ["SFR"]
        ...
        unit = string(units["mass"] / units["time"])
    ...
    elseif x_quantity ∈ ["P"]
        ...
        unit = string(units["mass"]^2 / units["length"]^4)

        if x_factor != 0
            x_unit = replace(
                replace(
                    unit, 
                    "M⊙^2" => "\\left(10^{$x_factor} \\, \\mathrm{M_{\\odot}^2} \\, \\mathrm{",
                ),
                "^-4" => "^{-4}}\\right)",
            )
        else
            x_unit = replace(
                replace(unit, "M⊙^2" => "\\left(\\mathrm{M_{\\odot}^2} \\, \\mathrm{"),
                "^-4" => "^{-4}}\\right)",
            )
        end
    ...
    if y_quantity ∈ ["STARS", "GAS", "COLD_GAS"]
    ...
    elseif y_quantity ∈ ["SSFR", "SFE", "Psi_FMOL"]

        unit = string(units["time"]^-1)

        if y_factor != 0
            y_unit = replace(
                "\\left(10^{$y_factor} \\, \\mathrm{$unit",
                "^-1" => "^{-1}}\\right)",
            )
        else
            y_unit = replace(
    ...
    elseif y_quantity ∈ ["P"]
        ...
        if y_factor != 0
            y_unit = replace(
                replace(
                    unit, 
                    "M⊙^2" => "\\left(10^{$y_factor} \\, \\mathrm{M_{\\odot}^2} \\, \\mathrm{",
                ),
                "^-4" => "^{-4}}\\right)",
            )
        else
    ...
    labels = Dict(
        "GAS" => "\\Sigma_{gas}",
        "COLD_GAS" => "\\Sigma_{cold \\ gas}",
        "STARS" => "\\Sigma_*",
        "OH" => "12 + \\log_{10}(O / H)",
        "SFR" => "SFR",
        "SSFR" => "SSFR",
        "SFE" => "SFE",
        "P" => "\\Sigma_{gas} \\, (\\Sigma_{gas} + \\Sigma_*)",
        "Psi_FMOL" => "\\Psi / H_2",
    )

    if x_unit === nothing
        xlabel = L"\mathrm{%$(labels[x_quantity])}"
    else
        xlabel = L"\mathrm{%$(labels[x_quantity])} \, / \, \mathrm{%$x_unit}"
    end

    if y_unit === nothing
        ylabel = L"\mathrm{%$(labels[y_quantity])}"
    else
        ylabel = L"\mathrm{%$(labels[y_quantity])} \, / \, \mathrm{%$y_unit}"
    end
...
Make a plot of molecular or atomic fraction vs. ``\mathrm{log}_{10}(T)``, 
for the gas particles at a given time step.

# Arguments
- `temperature_data::Dict{String,Any}`: Return value of the [`get_temperature`](@ref) function.
- `y_data::Dict{String, Any}`: Return value of the [`get_fmol`](@ref) or [`get_fatom`](@ref) function.
- `time::Unitful.Quantity`: Time with units for the time stamp of the plot. 
  All available time units in [Unitful](https://github.com/PainterQubits/Unitful.jl) and [UnitfulAstro](https://github.com/JuliaAstro/UnitfulAstro.jl) can be used, e.g. UnitfulAstro.Myr.
...
# Arguments
- `fatom_data::Vector{Float64}`: Return value of the [`get_fatom`](@ref) function.
- `density_data::Dict{String, Any}`: Return value of the [`get_density`](@ref) function.
- `time::Unitful.Quantity`: Time with units for the time stamp of the plot. 
  All available time units in [Unitful](https://github.com/PainterQubits/Unitful.jl) and [UnitfulAstro](https://github.com/JuliaAstro/UnitfulAstro.jl) can be used, e.g. UnitfulAstro.Myr.
...
Make a plot of molecular fraction vs. the metallicity, for the gas particles at a given 
time step.

# Arguments
- `fmol_data::Vector{Float64}`: Return value of the [`get_fmol`](@ref) function.
- `metallicity_data::Dict{String, Any}`: Return value of the [`get_metallicity`](@ref) 
  function.
- `mass_data::Dict{String, Any}`: Return value of the [`get_mass`](@ref) function, for gas.
- `time::Unitful.Quantity`: Time with units for the time stamp of the plot. 
  All available time units in [Unitful](https://github.com/PainterQubits/Unitful.jl) and [UnitfulAstro](https://github.com/JuliaAstro/UnitfulAstro.jl) can be used, e.g. UnitfulAstro.Myr.
...
function fmol_Z_plot(
    ...
    Z = metallicity_data["Z"]
    metallicity_unit = string(metallicity_data["unit"])

    mass = mass_data["mass"]
    mass_unit = string(mass_data["unit"])

    fmol = copy(fmol_data)

    # Unit consistency check
    (
        metallicity_unit == mass_unit || 
        error("The mass unit and the metallicity unit must be the same")
    )

    # Dimensionless metallicity
    z = (Z ./ mass) ./ SOLAR_METALLICITY

    # Data filtering for logarithmic scale
    deleteat!(fmol, z .<= 0.0)
    filter!(x -> x > 0.0, z)
    deleteat!(z, fmol .<= 0.0)
    filter!(x -> x > 0.0, fmol)
    ...
    figure = scatter(
        z,
        fmol;
        xlabel = L"\mathrm{Z / Z_\odot}",
        ylabel = L"\mathrm{f_{mol}}",
        xscale = scale,
        yscale = scale,
        size = (1000, 800),
        framestyle = :box,
    ...
    annotate!(
        relative(figure, 0.5, 0.95, log = (scale == :log10, scale == :log10))...,
        text("$clock $time_unit", "Courier", 22, :center),
    )
...
function fmol_fatom_plot(
    ...
    ρ = density_data["density"] .* density_data["unit"]
    nh0 = @. ustrip(Float64, Unitful.cm^(-3), (fatom_data * ρ) / Unitful.mp)
    ...
    # Particles with solar metallicity < 0.2
    # Equivalent to z = 0.1 in Gnedin2009
    idx_01 = findall(x -> x < 0.2, z)
    nh0_01 = nh0[idx_01]
    fmol_01 = fmol[idx_01]
    
    # Particles with solar metallicity < 0.65 and >= 0.2
    # Equivalent to z = 0.3 in Gnedin2009
    idx_02 = findall(x -> 0.2 <= x < 0.65, z)
    nh0_02 = nh0[idx_02]
    fmol_02 = fmol[idx_02]
    
    # Particles with solar metallicity >= 0.65
    # Equivalent to z = 1 in Gnedin2009
    idx_03 = findall(x -> 0.65 <= x, z)
    nh0_03 = nh0[idx_03]
    fmol_03 = fmol[idx_03]
    ...
    figure = scatter(
        hcat(x_data...),
        hcat(y_data...);
        xlabel = L"\mathrm{n_{H}} \, / \, \mathrm{cm^{-3}}",
        ylabel = L"\mathrm{f_{mol}}",
        xscale = scale,
        size = (1000, 800),
        framestyle = :box,
        label = [L"\mathrm{z < 0.2}" L"\mathrm{0.2 \leq z < 0.65}" L"\mathrm{z \geq 0.65}"],
        legend = :topleft,
        background_color_legend = nothing,
        foreground_color_legend = nothing,
        markersize = 3,
